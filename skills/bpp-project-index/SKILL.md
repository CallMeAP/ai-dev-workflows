...
Regenerates the GitLab-only section by diffing the full `lipso/clients/brokernet/` group (incl. subgroups, non-archived) against local checkouts under both roots. Requires `glab` + `jq`.
...
```bash
{
  ...
  echo
  echo 'Repos existing in the GitLab `lipso/clients/brokernet/` group (incl. subgroups, non-archived) with no local checkout. Clone target: `bpp-*` → `~/Entwicklung/bpp/{name}`, else `~/Entwicklung/brokernet/{name}`.'
  echo
