...
Path lookup reference for all BPP (.NET + Java) and legacy Brokernet (Java/Maven) projects on this machine. Read the index whenever a project name is mentioned to resolve its path and purpose before exploring or editing code.

## When to Use

- User names a BPP repo (`bpp-*`) or Brokernet module (`backend-*`, `brokernet-*`).
- User asks "where is X", "what is Y", "path to Z" for BPP/Brokernet code.
...
- Two roots only: `bpp/` (new) and `brokernet/` (legacy).
