...
| Where bpp-* skills go | `skills/<name>/` | `bpp/skills/<name>/` (shared) — **deliberately a different path, do not "fix"** |
| Where personal skills go (`gs-*`, `lipsum-stundenliste`, `sync-my-skills`) | `skills/<name>/` | `personal-workflows/apittrich/skills/<name>/` — **never** the shared `bpp/skills/` |
...
**The two shared-skill paths differ on purpose.** Canonical keeps a top-level `skills/`; the mirror
nests them under `bpp/skills/` (moved 2026-09-14), because the mirror also carries `angular/`,
`.net/` and `personal-workflows/` and the shared skills are BPP-specific. **Do not "align" the two.**
A sync that writes a `bpp-*` skill to the mirror's top-level `skills/` puts it where `install.sh`
does not look, and every other developer's `install.sh … skills all` silently stops seeing it.
The mirror's `install.sh` is the authority for this path — read it, do not assume (step 2).
...
### 2. Discover where skills live in each repo (don't hardcode)

Both repos contain **several** directories named `skills` (the mirror has `bpp/skills`,
`.net/bpp-stella/skills` and one per developer under `personal-workflows/`). Take the one that
actually holds the `bpp-*` skills, never the first `find` hit.

Canonical — a top-level `skills/`:

```bash
find /home/alex/Entwicklung/ai-dev-workflows -maxdepth 2 -type d -name skills -not -path '*/.git/*'
```

Mirror — ask `install.sh`, which is the renderer and therefore cannot drift from the truth:

```bash
grep -n 'render_tree "$here' /home/alex/Entwicklung/lipso/agentic-coding-knowledge/install.sh
```

The path it renders from **is** the mirror's shared skills root. At time of writing: canonical
`ai-dev-workflows/skills/`, mirror `agentic-coding-knowledge/bpp/skills/`, personal skills in the
mirror at `.../personal-workflows/apittrich/skills/` — **but confirm from the repo, don't trust this
line.**
...
- **Reverse-render on the way to the mirror** — a `bpp-*` skill goes to the shared `bpp/skills/` with every
  ...
  sed -e "s|{{BPP_ROOT}}|$BPP_ROOT|g" -e "s|{{BROKERNET_ROOT}}|$BROKERNET_ROOT|g" \
      ...
      -e "s|{{KNOWLEDGE_DIR}}|$KNOWLEDGE_DIR|g" "$K/bpp/skills/$NAME/SKILL.md" \
  ...
  `grep -rn '/home/' "$K/bpp/skills/"` must be empty.
- **New shared skill** → write its `bpp/skills/<name>/README.md` in the same change. A skill reaching the
...
- **Personal skill** (`gs-*`, `lipsum-stundenliste`, `sync-my-skills`) → mirror copy goes to
  ...
  `bpp/skills/`.
- **Support files carry the same contract as the `.md`.** Under the shared `bpp/skills/` a `.sh`/`.py`
...
git -C "$MIRROR" add "personal-workflows/apittrich/skills/$NAME/"   # or bpp/skills/$NAME/ for a shared one
...
| Skills path | canonical `skills/`, mirror `bpp/skills/` — **asymmetric on purpose**; confirm both per step 2, don't hardcode |
...
| Placeholder check | render mirror copy back with `paths.env`, diff vs live; `grep -rn '/home/' bpp/skills/` empty |
| Personal skills | canonical `skills/`, mirror `personal-workflows/apittrich/skills/` — never shared `bpp/skills/` |
...
- **Pushing literal `/home/<you>/...` paths into the mirror's shared `bpp/skills/`** → `install.sh` renders placeholders, so a hardcoded path silently installs your machine's layout on someone else's. Reverse-render, then render back and diff.
...
- A `{{PLACEHOLDER}}` about to reach the **canonical** repo, or a literal `/home/...` path about to reach the mirror's shared `bpp/skills/` → STOP; you have the two contracts swapped.
- About to write a `bpp-*` skill to the mirror's **top-level** `skills/` → STOP; the mirror's shared root is `bpp/skills/`. The old path is not tracked any more and `install.sh` will not find the skill.
